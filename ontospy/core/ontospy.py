...
from .entities import RdfEntity, Ontology, OntoClass, OntoProperty, OntoSKOSConcept, OntoShape
...
class _Ontospy:
    ...
    credentials = None  # tuple: auth credentials for endpoint if needed
    sources: list = None
    sparqlHelper = None
    pref_title: str
    pref_lang: str = "en"
    ...
    all_classes: list = []
    all_properties: list = []
    all_properties_annotation: list = []
    all_properties_object: list = []
    all_properties_datatype: list = []
    all_skos_concepts: list = []
    all_shapes: list = []
    all_individuals: list = []
    toplayer_classes: list = []
    toplayer_properties: list = []
    toplayer_skos: list = []
    toplayer_shapes: list = []
    OWLTHING: OntoClass
    ...
    def __repr__(self):
        ...
        if self.sparql_endpoint and self.rdflib_graph != None:
            return f"<Ontospy Graph (sparql endpoint = <{self.sparql_endpoint}>)>"
        elif self.rdflib_graph != None:
            return f"<Ontospy Graph ({len(self.rdflib_graph):d} triples)>"
        else:
            return "<Ontospy object created but not initialized (use the `load_rdf` method to load an rdf schema)>"
    
    def load_rdf(
            self, uri_or_path=None, data=None, file_obj=None, rdf_format="", verbose=False
    ):
    ...
    def load_sparql(self, sparql_endpoint, verbose=False, credentials=None):
        ...
        try:
            ...
            ut.printDebug("Error trying to connect to Endpoint.")
            raise
    ...
    def build_all(
        ...
        """
        if verbose:
            ut.printInfo("Scanning entities...", "green")
            ut.printInfo("----------", "comment")
        
        self.build_ontologies()
        if verbose:
            ut.printInfo(f"Ontologies.........: {len(self.all_ontologies):d}", "comment")
        
        self.build_classes(hide_base_schemas, hide_implicit_types)
        if verbose:
            ut.printInfo(f"Classes............: {len(self.all_classes):d}", "comment")
        
        self.build_properties(hide_implicit_preds)
        if verbose:
            ut.printInfo(f"Properties.........: {len(self.all_properties):d}", "comment")
        if verbose:
            ut.printInfo(
                    f"..annotation.......: {len(self.all_properties_annotation):d}",
                    "comment",
            )
        if verbose:
            ut.printInfo(
                    f"..datatype.........: {len(self.all_properties_datatype):d}", "comment"
            )
        if verbose:
            ut.printInfo(
                    f"..object...........: {len(self.all_properties_object):d}", "comment"
            )
        
        self.build_skos_concepts()
        if verbose:
            ut.printInfo(
                    f"Concepts (SKOS)....: {len(self.all_skos_concepts):d}", "comment"
            )
        
        self.build_shapes()
        if verbose:
            ut.printInfo(f"Shapes (SHACL).....: {len(self.all_shapes):d}", "comment")
        
        if not hide_individuals:
            self.build_individuals()
            if verbose:
                ut.printInfo(
                        f"Individuals........: {len(self.all_individuals):d}", "comment"
                )
    ...
    def build_ontologies(self, exclude_BNodes=False, return_string=False):
        ...
        if qres:
            ...
            for candidate in qres:
                if ut.isBlankNode(candidate[0]):
                    if exclude_BNodes:
                        continue
                    else:
                        checkDC_ID = [
                                x
                                for x in self.rdflib_graph.objects(
            ...
            pass
            # ut.printInfo("No owl:Ontologies found")
    ...
    def build_classes(self, hide_base_schemas=True, hide_implicit_types=True):
        ...
        qres = self.sparqlHelper.getAllClasses(hide_base_schemas, hide_implicit_types)
        ...
        for class_tuple in qres:
            ...
            if not test_existing_cl:
                # create it
                ontoclass = OntoClass(
                        _uri, _type, self.namespaces, False, self.pref_title, self.pref_lang
                )
    ...
    def build_properties(self, hide_implicit_preds=True):
        ...
        for candidate in qres:
            ...
            if not test_existing_prop:
                ...
                if candidate[1] and (test_existing_prop.rdftype == rdflib.RDF.Property):
                    test_existing_prop.rdftype = ut.inferMainPropertyType(candidate[1])
        ...
        self.toplayer_properties = (
                exit  # sorted(exit, key=lambda x: x.id) # doesnt work
        )
            ...
            for uri in aShape.getValuesForProperty(shacl["targetClass"]):
                ...
                if aclass:
                    ...
                    for propertyUri in aShape.getValuesForProperty(shacl["path"]):
                        ...
                        if propType:
                            aclass.shapedProperties += [
                                    {"shape": aShape, "property": propType}
                            ]
    ...
    def build_entity_from_uri(self, uri, ontospyClass=None):
        ...
        if not ontospyClass:
            ...
            ut.printDebug(
                    f"Error: <{str(ontospyClass)}> is not a subclass of ontospy.RdfEntity"
            )
            return None
        else:
            pass
    ...
    def __buildDomainRanges(self, aProp):
        ...
        domains = chain(
                aProp.rdflib_graph.objects(
                        None, rdflib.term.URIRef("http://schema.org/domainIncludes")
                ),
                aProp.rdflib_graph.objects(None, rdflib.RDFS.domain),
        )
        
        ranges = chain(
                aProp.rdflib_graph.objects(
                        None, rdflib.term.URIRef("http://schema.org/rangeIncludes")
                ),
                aProp.rdflib_graph.objects(None, rdflib.RDFS.range),
        )
    ...
    def get_class(self, id=None, uri=None, match: str=None):
        """
        ...
        return self._get_defined_entity(
                id=id, uri=uri, match=match, candidates_list=self.all_classes
        )
    
    def get_property(self, id=None, uri=None, match: str=None):
        """Get the saved-property with given ID or via other methods..."""
        return self._get_defined_entity(
                id=id, uri=uri, match=match, candidates_list=self.all_properties
        )
    
    def get_individual(self, id=None, uri=None, match: str=None):
        """Get the saved-individual with given ID or via other methods..."""
        return self._get_defined_entity(
                id=id, uri=uri, match=match, candidates_list=self.all_individuals
        )
    
    def get_skos(self, id=None, uri=None, match: str=None):
        """Get the saved skos concept with given ID or via other methods..."""
        return self._get_defined_entity(
                id=id, uri=uri, match=match, candidates_list=self.all_skos_concepts
        )
    
    def get_shapes(self, id=None, uri=None, match: str=None):
        """Get the saved shacl shapes with given ID or via other methods..."""
        return self._get_defined_entity(
                id=id, uri=uri, match=match, candidates_list=self.all_shapes
        )
    
    def get_ontology(self, id=None, uri=None, match: str=None):
        """Get the saved-ontology with given ID or via other methods..."""
        return self._get_defined_entity(
                id=id, uri=uri, match=match, candidates_list=self.all_ontologies
        )
    
    def get_any_entity(self, id=None, uri=None, match: str=None):
        ...
        return self._get_defined_entity(
                id=id, uri=uri, match=match, candidates_list=all_entities
        )
    ...
    def triplesCount(self):
        ...
        # ut.printDebug(unicode(type(self.rdflib_graph)), fg="red")
        try:
            return len(self.rdflib_graph)
        except:
            ut.printDebug("Ontospy: error counting graph length..", fg="red")
            return 0
    ...
    def printClassTree(self, element=None, showids=False, labels=False, showtype=False):
        ...
        if not element:  # first time
            for x in self.toplayer_classes:
                ut.printGenericTree(x, 0, showids, labels, showtype, TYPE_MARGIN)
        
        else:
            ut.printGenericTree(element, 0, showids, labels, showtype, TYPE_MARGIN)
    
    def printPropertyTree(
            self, element=None, showids=False, labels=False, showtype=False
    ):
        """
...
class Ontospy:
    ...
    def __init__(
            ...
            rdf_format="",
            verbose: bool = False,
            hide_base_schemas: bool = True,
            hide_implicit_types: bool = True,
            hide_implicit_preds: bool = True,
            hide_individuals: bool = True,
            sparql_endpoint=None,
            credentials=None,
            build_all: bool = True,
            pref_title="qname",
        ...
        self.pref_title: str = pref_title
        ...
        self.OWLTHING = OntoClass(uri=rdflib.OWL.Thing,
                                  rdftype=rdflib.OWL.Class,
                                  namespaces=self.namespaces,
                                  ext_model=False,
                                  pref_title=self.pref_title,
                                  pref_lang=self.pref_lang,
    ...
    def load_rdf(self, uri_or_path=None, data=None, file_obj=None, rdf_format="", verbose=False):
        """Load an RDF source into an ontospy/rdflib graph"""
        loader = self.loader = RDFLoader(verbose=verbose)
        loader.load(uri_or_path, data, file_obj, rdf_format)
        self.rdflib_graph = loader.rdflib_graph
        self.sources = loader.sources_valid
        self.sparqlHelper = SparqlHelper(self.rdflib_graph)
    ...
    def load_sparql(self, sparql_endpoint: str, verbose=False, credentials=None):
        """
        Set up a SPARQLStore backend as a virtual ontospy graph
