...
def printComparison(results: dict, class_or_prop):
    """
    ...
    for k, v in sorted(list(results.items()), key=lambda x: x[1]):
        data += [Row(k, str(v))]
    pprinttable(data)
...
def main():
    ...
    if ontology and somegraph:
        ...
        print((f"Graph to validate: <{somegraph}>\n"))
